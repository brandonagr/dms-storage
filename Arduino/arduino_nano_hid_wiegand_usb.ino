...
const byte MY_ADDRESS = 15;          // Address for i2c Slave
const byte OTHER_ADDRESS = 10;       // Address for i2c Master

#define MAX_BITS 100                 /* max number of bits */ 
#define WIEGAND_WAIT  3000           /* time to wait for another wiegand pulse */ 
...
unsigned char flagType;              // stores type of input, C for card, P for pin
unsigned int  wiegand_counter=WIEGAND_WAIT;  // countdown until we assume there are no more bits
unsigned long lastTime=0;            // millis the last time a digit pressed
...
// int parity(int from, int to, int startingparity)
// {
//   for (i = from; i <= to; i++) {
//     if (databits[i] == 1) {
//        startingparity = (startingparity = 0 ? 1 : 0);
//     }
//   }
//   return startingparity;
// }
    ...
    else if (bitCount == 26)
    {
      ...
      if (evenparity != databits[0])
      {
         Serial.println("");
         Serial.println ("Bad Even Parity");
      }
      Serial.print ("Even Parity ");
      Serial.print (evenparity);
      Serial.print (" = ");
      Serial.println (databits[0]);
      ...
      if (oddparity != databits[25])
      {
         Serial.println ("");
         Serial.println ("Bad Odd Parity");
      }
      Serial.print ("Odd Parity ");
      Serial.print (oddparity);
      Serial.print (" = ");
      Serial.print (databits[25]);
    ...
    else if (bitCount == 4)
    {
      ...
      if (internalCode <= 9 || internalCode == KEYPAD_ENT)
        ...
        currentTime = millis();
        if (currentTime >= lastTime + 1000 * SECONDS_UNTIL_CLEAR)
          {
          // Clear pending PIN
          pinCode = 0;
          pinKeys = 0;
